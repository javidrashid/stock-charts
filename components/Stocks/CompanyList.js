//This is an Incomplete Component built for AutoComplete API Call, It is not part of the main code that is deployed.
          ...
          <div id="app" className="flex justify-center mt-12">
            <div>
              <div className="w-full relative">
                ...
                <div className="mt-1 relative border border-gray-300 overflow-hidden rounded-md shadow-sm">
                  <input
                    id="email"
                    className="w-full px-3 py-3"
                    autoComplete="off"
                    placeholder="search for company symbol"
                    onChange={(e) => handleChange(e)}
                  />
                </div>
                <div className="absolute w-full z-50 bg-white border border-gray-300 mt-1 max-height-48 overflow-hidden overflow-y-scroll rounded-md shadow-md">
                  <ul className="py-1">
                    <li className="px-3 py-2 cursor-pointer hover:bg-gray-200">
                      {companylist.map((c) => (
                        <p key={c[0]} onClick={updateChart(c)}>
                          {c[1]}
                        </p>
                      ))}
                    </li>
